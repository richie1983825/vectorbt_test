...
from strategies.polyfit_switch import generate_grid_priority_switch_signals_v7, generate_grid_priority_switch_signals_v8
...
SWITCH_V8_PARAMS_KEYS = SWITCH_V7_PARAMS_KEYS + [
    "enable_panic_dip_buy", "panic_ret_5d_max",
    "panic_amplitude", "panic_vol_ratio",
]
...
# V8 只扫描恐慌低吸参数
SW_V8_PANIC_SCAN = {
    "enable_panic_dip_buy": [True, False],
    "panic_ret_5d_max": [0.03, 0.05, 0.07],
    "panic_amplitude": [0.025, 0.03, 0.04],
    "panic_vol_ratio": [1.3, 1.5, 2.0],
}
SW_V8_KEYS = list(SW_V8_PANIC_SCAN.keys())
SW_V8_COMBOS = list(product(*SW_V8_PANIC_SCAN.values()))


def _make_switch_v8_eval(open_raw, high_raw=None, low_raw=None, volume_raw=None):
    def _eval(close_warmup_all, test_offset, params):
        ind = compute_polyfit_switch_indicators(
            close_warmup_all, fit_window_days=252, ma_windows=[20, 60],
            trend_window_days=int(params.get("trend_window_days", 20)),
        ...
        e_sw, x_sw, s_sw = generate_grid_priority_switch_signals_v8(
            cl_test.values, ind_test["PolyDevPct"].values, ind_test["PolyDevTrend"].values,
            ind_test["RollingVolPct"].values, ind_test["PolyBasePred"].values,
            e_grid, x_grid, ind_test["MA20"].values, ind_test["MA60"].values,
            trend_entry_dp=V6_FIXED["trend_entry_dp"],
            ...
            enable_ohlcv_filter=V6_FIXED["enable_ohlcv_filter"],
            enable_early_exit=V6_FIXED["enable_early_exit"],
            enable_top_avoidance=V8_FIXED_TOP["enable_top_avoidance"],
            top_ret_5d=V8_FIXED_TOP["top_ret_5d"],
            top_price_pos=V8_FIXED_TOP["top_price_pos"],
            top_amplitude=V8_FIXED_TOP["top_amplitude"],
            top_block_days=V8_FIXED_TOP["top_block_days"],
            enable_panic_dip_buy=bool(params.get("enable_panic_dip_buy", True)),
            panic_ret_5d_max=params.get("panic_ret_5d_max", 0.05),
            panic_amplitude=params.get("panic_amplitude", 0.03),
            panic_vol_ratio=params.get("panic_vol_ratio", 1.5),
            high=h_test, low=l_test, open_=op_test.values if op_test is not None else None,
            volume=v_test,
        )
...
def _scan_switch_v8_train(close_train, open_, high, low, volume, grid_params):
    """训练期 CPU 扫描 Switch-v8 参数（V7顶部固定 + V8恐慌扫描）。"""
    tw_s = int(grid_params["trend_window_days"])
    vw_s = int(grid_params["vol_window_days"])
    ...
    results = []
    for combo in SW_V8_COMBOS:
        kw = dict(zip(SW_V8_KEYS, combo))
        e_sw, x_sw, s_sw = generate_grid_priority_switch_signals_v8(
            cl_arr, indicators["PolyDevPct"].values, indicators["PolyDevTrend"].values,
            indicators["RollingVolPct"].values, indicators["PolyBasePred"].values,
            e_grid, x_grid, indicators["MA20"].values, indicators["MA60"].values,
            ...
            top_amplitude=V8_FIXED_TOP["top_amplitude"],
            top_block_days=V8_FIXED_TOP["top_block_days"],
            enable_panic_dip_buy=kw["enable_panic_dip_buy"],
            panic_ret_5d_max=kw["panic_ret_5d_max"],
            panic_amplitude=kw["panic_amplitude"],
            panic_vol_ratio=kw["panic_vol_ratio"],
            high=h_train, low=l_train, open_=op_arr, volume=v_train,
        )
        e_merged = e_grid | e_sw
        x_merged = x_grid | x_sw
...
def run_switch_v8_wf(close, open_=None, high=None, low=None, volume=None,
                     windows=None, grid_wf_df=None,
                     train_months=22, test_months=12, step_months=3, warmup_months=12,
                     verbose=True):
    """运行 Polyfit-Switch-v8 Walk-Forward 扫描（V7顶部固定 + V8恐慌扫描）。"""
    from utils.walkforward import generate_monthly_windows
    ...
    switch_eval = _make_switch_v8_eval(open_, high, low, volume)
    wf_rows = []
    n_proc = 0; t0 = time.time()
    ...
    for wi, w in enumerate(windows):
        ...
        for sel_name, sel_fn in SELECTORS.items():
            ...
            sw_df = _scan_switch_v8_train(close_train, open_, high, low, volume, grid_params)
            if sw_df.empty: continue
            ...
            try:
                ...
                continue
            sw_params = {k: best_sw[k] for k in SWITCH_V8_PARAMS_KEYS if k in best_sw.index}
            sw_oos = switch_eval(close_warmup_all, test_offset, sw_params)
            wf_rows.append({
                "strategy": "Polyfit-Switch-v8", "train_months": train_months,
                "selector": sel_name, "n_train_bars": n_train_bars,
                "test_start_date": test_close.index[0],
                "test_end_date": test_close.index[-1],
                "train_return": best_sw["total_return"],
        ...
        if verbose and ((wi + 1) % 5 == 0 or wi == 0):
            elapsed = time.time() - t0
            print(f"  [Switch-v8 WF: {wi+1}/{len(windows)}] {elapsed:.0f}s")

    wf_df = pd.DataFrame(wf_rows)
    if verbose:
        elapsed = time.time() - t0
        print(f"  Switch-v8 WF done: {len(wf_df)} rows, {elapsed:.0f}s")
    return wf_df
