...
class PolyfitDynamicGridStrategy(Strategy):
    """基于3年线性拟合基准的动态阈值网格策略（仅做多）。"""
    ...
    def _build_entry_reason(self, close: float, pred: float, dev_pct: float, dynamic_grid_step: float, entry_level: int) -> str:
        return (
    ...
    def next(self) -> None:
        ...
        if any(np.isnan(v) for v in [close, pred, dev_pct, dev_trend, rolling_vol_pct]) or pred <= 0:
            return
        ...
        if not self.position:
            ...
            if dev_pct <= entry_threshold and signal_strength >= float(self.min_signal_strength):
                ...
                self.current_entry_reason = self._build_entry_reason(close, pred, dev_pct, dynamic_grid_step, entry_level)
            return
